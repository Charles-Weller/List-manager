...
public class Main extends JFrame implements ActionListener {
    ...
    public boolean filesEnabled = false;  // change this to true, to enable loading and saving data from/to a file
...
// ------------------- GUI set up ---------------//
    ...
    public void updateListView() {
        ...
        // Called initially when the GUI is set-up the first time, and afterwards, is called every time the GUI is redrawn.
...
// --------------- code for handling events ---------------//
    ...
    private void clickedItem(int i) {
        ...
        if (chosen.equals("done")) {
            ...
            if (i < firstListSize) {
                lists.changePriority(false, i);
            } else {
                lists.changePriority(true, i - firstListSize);
            }
            grabData();
            redraw();
...
// --------------- various graphics drawing methods ---------------//
    ...
    public BufferedImage getItemImage(int width, String task, boolean important) {
        ...
        Rectangle2D box = gl.getVisualBounds();
        if (box.getWidth() + offsetX + 3 > width) {  
